...
import { projectForAuthorizedShare, type ShareProjection } from "../core/share";
...
export async function mountApp(root: HTMLElement, store: CanonicalStore, commands: CommandBus, capabilityRuntime?: CapabilityRuntime<unknown>, packageAutomationRuntime?: PackageAutomationRuntime, options: MountAppOptions = {}): Promise<void> {
  ...
  let pendingShareProjection: ShareProjection | undefined;
  ...
  root.innerHTML = `
    ...
    <main>
      <section class="status-card" aria-labelledby="status-heading">
        <div>
          ...
          <details id="health-details" class="health-details compact-panel">
            <summary class="compact-summary"><span class="compact-summary-copy"><span>${copy.system}</span></span></summary>
            <p id="health-detail-status">${copy.healthInitial}</p>
          </details>
        </div>
      ...
      <dialog id="record-detail-dialog" aria-labelledby="record-detail-heading" aria-describedby="record-detail-context">
        <div class="record-detail-content">
          ...
          <div class="form-row">
            <button id="record-detail-assistant" class="secondary" type="button">${copy.assistantOpen}</button>
      ...
      <details id="assistant" class="panel compact-panel" aria-labelledby="assistant-heading" data-section-disclosure>
        <summary class="compact-summary"><span class="compact-summary-copy"><p class="eyebrow">${copy.assistant}</p><h2 id="assistant-heading">${copy.assistantHeading}</h2></span><span id="assistant-route-pill" class="status-pill">${copy.assistantProvider}</span></summary>
        <p class="hint">${copy.assistantHint}</p>
        <div class="assistant-boundary" role="status">
          <p><strong>${copy.assistantScope}:</strong> <span id="assistant-scope">${copy.assistantStandaloneScope}</span></p>
          <p><strong>${copy.assistantProvider}</strong></p>
          <p id="assistant-disclosure" class="hint">${copy.assistantDisclosure}</p>
        </div>
        <form id="assistant-form" class="relationship-form">
          <label for="assistant-question">${copy.assistantQuestion}</label>
          <textarea id="assistant-question" rows="3" maxlength="5000" placeholder="${copy.assistantQuestionPlaceholder}"></textarea>
          <div class="form-row">
            <button id="assistant-submit" type="submit" disabled>${copy.assistantSubmit}</button>
            <button id="assistant-clear-scope" class="secondary" type="button">${copy.assistantClearScope}</button>
          </div>
          <p id="assistant-status" class="hint" role="status">${copy.assistantDisabled}</p>
        </form>
      </details>
      ...
      <details id="recovery" class="panel compact-panel" aria-labelledby="recovery-heading" data-section-disclosure>
        ...
        <details id="recovery-backup-tools" class="relationship-form compact-panel recovery-tool-group">
          ...
          <form id="recovery-backup-form" class="relationship-form" autocomplete="off">
            <p class="hint">${copy.recoveryHint}</p>
            ...
            <input id="vault-password-username" name="username" type="text" autocomplete="username" hidden />
            <label for="vault-password">${recoveryCopy.password}</label>
            <input id="vault-password" name="vaultPassword" type="password" minlength="8" autocomplete="new-password" />
    ...
    <dialog id="share-preview-dialog" aria-labelledby="share-preview-heading" aria-describedby="share-preview-hint" aria-modal="true">
      <h2 id="share-preview-heading">${copy.sharePreviewHeading}</h2>
      <p id="share-preview-hint" class="hint">${copy.sharePreviewHint}</p>
      <p id="share-preview-summary" class="hint" role="status"></p>
      <pre id="share-preview-payload" class="share-preview-payload" tabindex="0"></pre>
      <p id="share-preview-excluded" class="hint">${copy.sharePreviewExcluded}</p>
      <div class="dialog-actions">
        <button id="share-preview-cancel" class="secondary" type="button">${copy.sharePreviewCancel}</button>
        <button id="share-preview-confirm" type="button">${copy.sharePreviewConfirm}</button>
      </div>
    </dialog>
  ...
  for (const control of root.querySelectorAll<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>("input, select, textarea")) {
    if (!control.hasAttribute("autocomplete") && (control.name || control.type === "password")) control.setAttribute("autocomplete", "off");
  }
  ...
  const assistantQuestion = root.querySelector<HTMLTextAreaElement>("#assistant-question");
  const assistantSubmit = root.querySelector<HTMLButtonElement>("#assistant-submit");
  const assistantClearScope = root.querySelector<HTMLButtonElement>("#assistant-clear-scope");
  const assistantScope = root.querySelector<HTMLElement>("#assistant-scope");
  const assistantStatus = root.querySelector<HTMLElement>("#assistant-status");
  const assistantSection = root.querySelector<HTMLDetailsElement>("#assistant");
  ...
  const sharePreviewDialog = root.querySelector<HTMLDialogElement>("#share-preview-dialog");
  const sharePreviewSummary = root.querySelector<HTMLElement>("#share-preview-summary");
  const sharePreviewPayload = root.querySelector<HTMLElement>("#share-preview-payload");
  const sharePreviewCancel = root.querySelector<HTMLButtonElement>("#share-preview-cancel");
  const sharePreviewConfirm = root.querySelector<HTMLButtonElement>("#share-preview-confirm");
  ...
  const recordDetailAssistant = root.querySelector<HTMLButtonElement>("#record-detail-assistant");
  ...
  const healthDetails = root.querySelector<HTMLDetailsElement>("#health-details");
  const healthDetailStatus = root.querySelector<HTMLElement>("#health-detail-status");
  ...
  const recoveryBackupForm = root.querySelector<HTMLFormElement>("#recovery-backup-form");
  ...
  recoveryBackupForm.addEventListener("submit", (event) => event.preventDefault());
  ...
  let assistantScopeRecordId: string | undefined;
  let assistantScopeRelationshipCount = 0;
  ...
  const sectionIcon = (id: PresentationSectionId): string => ({ "home-summary": "⌂", capture: "✎", acquire: "↓", track: "◌", domains: "◇", search: "⌕", assistant: "✦", spaces: "▦", compose: "▤", review: "✓", relate: "↔", knowledge: "§", sharing: "⇧", sync: "⟳", focus: "◷", reminders: "!", records: "☷", recovery: "↺", presentation: "⚙" })[id];
  ...
  const applyPresentationProfile = (): void => {
    ...
    healthDetails.open = presentation.density !== "compact";
  ...
  const renderAssistantScope = async (): Promise<void> => {
    if (!assistantScopeRecordId) {
      assistantScope.textContent = copy.assistantStandaloneScope;
      assistantClearScope.disabled = true;
      return;
    }
    const record = await store.get(assistantScopeRecordId, true);
    if (!record) {
      assistantScopeRecordId = undefined;
      assistantScopeRelationshipCount = 0;
      assistantScope.textContent = copy.assistantStandaloneScope;
      assistantClearScope.disabled = true;
      return;
    }
    assistantScope.textContent = copy.assistantRecordScope(record.id, assistantScopeRelationshipCount);
    assistantClearScope.disabled = false;
  };
  const openAssistantSurface = async (recordId?: string, relationshipCount = 0): Promise<void> => {
    assistantScopeRecordId = recordId;
    assistantScopeRelationshipCount = recordId ? relationshipCount : 0;
    if (!presentation.navigation.visible.includes("assistant")) {
      await persistPresentation(parsePresentationProfile({ ...presentation, navigation: { ...presentation.navigation, visible: [...presentation.navigation.visible, "assistant"] } }), copy.savedName(copy.assistant));
    }
    await renderAssistantScope();
    assistantSection.open = true;
    assistantSection.scrollIntoView({ block: "start" });
  ...
  recordDetailAssistant.addEventListener("click", () => {
    recordDetailDialog.close();
    void openAssistantSurface(assistantScopeRecordId, assistantScopeRelationshipCount);
  });
  recordDetailClose.addEventListener("click", () => recordDetailDialog.close());
  assistantClearScope.addEventListener("click", () => {
    assistantScopeRecordId = undefined;
    assistantScopeRelationshipCount = 0;
    void renderAssistantScope();
  });
  assistantForm.addEventListener("submit", (event) => {
    event.preventDefault();
    assistantStatus.textContent = copy.assistantDisabled;
  });
  ...
  const renderTelemetryThresholdEditor = (): HTMLDetailsElement => {
    ...
    backupAge.id = "telemetry-backup-age";
    backupAge.name = "backupAgeDays";
    ...
    outbox.id = "telemetry-outbox-threshold";
    outbox.name = "pendingEffects";
    ...
    conflicts.id = "telemetry-conflict-threshold";
    conflicts.name = "unresolvedConflicts";
  ...
  const renderRecords = async (query = ""): Promise<number> => {
    ...
    healthDetailStatus.textContent = formatHealth(healthAfter);
    await renderEffects();
  ...
  shareExport.addEventListener("click", async () => {
    ...
    try {
      ...
      pendingShareProjection = projection;
      sharePreviewSummary.textContent = copy.sharePreviewSummary(projection.records.length, recordIds.length, projection.omittedRecordCount);
      sharePreviewPayload.textContent = serializeJson(projection);
      sharePreviewDialog.showModal();
      sharePreviewConfirm.focus();
    } catch (error) {
      shareStatus.textContent = describeError(error, "Bounded share projection failed; canonical records were not changed.");
    }
  });

  const cancelSharePreview = (): void => {
    pendingShareProjection = undefined;
    sharePreviewPayload.textContent = "";
    if (sharePreviewDialog.open) sharePreviewDialog.close();
  };
  sharePreviewCancel.addEventListener("click", cancelSharePreview);
  sharePreviewDialog.addEventListener("cancel", (event) => { event.preventDefault(); cancelSharePreview(); });
  sharePreviewConfirm.addEventListener("click", () => {
    const projection = pendingShareProjection;
    if (!projection) return cancelSharePreview();
    downloadJson("omnevum-share-projection.json", projection);
    shareStatus.textContent = copy.sharePreviewConfirmed;
    cancelSharePreview();
  });
  ...
  requestPersistenceButton.addEventListener("click", async () => {
    try {
      ...
      healthDetailStatus.textContent = formatHealth(health);

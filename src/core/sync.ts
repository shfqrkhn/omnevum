...
export class SyncEngine {
  ...
  public async synchronize(): Promise<SyncRunResult> {
    ...
    const imported = await this.store.importVault({ format: "OMNEVUM_VAULT", version: 1, exportedAt: new Date().toISOString(), records: merged.records });
    await this.transport.push(merged.records);
    return { ...merged, imported: imported.imported, skipped: imported.skipped, importConflicts: imported.conflicts };
  }
}

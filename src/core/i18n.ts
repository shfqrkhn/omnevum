...
export interface UiCopy {
  ...
  home: string; currentPicture: string; activeRecordCount: string; visualize: string; signals: string; openTasks: string; completedTasks: string; focusMinutes: string; relationships: string; personalization: string; makeItYours: string;
  derivedStatus: (records: number, sourceIds: number, groups: number) => string;
  appName: string; language: string; english: string; french: string; save: string; presentationHint: string;
  capture: string; getItOut: string; kind: string; note: string; task: string; observation: string; space: string; acquireHeading: string; stageImport: string; acceptStaged: string; acquirePlaceholder: string; acquireHint: string; stagedMessage: (count: number, warnings: number) => string;
  personal: string; household: string; work: string; captureContent: string; capturePlaceholder: string;
  captureHint: string; saveCapture: string; searchExplore: string; findCaptures: string; searchTerms: string;
  ...
  inboxClear: string; markReviewed: string; relate: string; connectWithoutCopying: string; sourceRecord: string;
  ...
  attachedMessage: (name: string, size: number) => string; startedMessage: (time: string) => string;
  savedFocusMessage: (minutes: number) => string; healthMessage: (active: number, archived: number, history: number, artifacts: number, index: string) => string;
  resultMessage: (count: number, index: string) => string;
}

const english: UiCopy = {
  scopeWithoutCopying: "Bound records to a Space", assignToSpace: "Record", filterSpace: "View Space", allSpaces: "All Spaces", addMembership: "Add to Space", membershipCreated: (space) => `Record added to ${space}; the canonical record was not copied.`, historyHeading: "Revision history", historyEntry: (revision, time, changes) => `Revision ${revision} - ${time}; changed: ${changes}`,
  ...
  productHeading: "Life, in context.", foundation: "foundation", lede: "A private, local-first place to capture what matters.", system: "System", ready: "Local foundation ready", local: "LOCAL", healthInitial: "Canonical records stay in this browser until you export them.", healthy: "healthy", degraded: "degraded", home: "Home", currentPicture: "Your current picture", activeRecordCount: "active record count", visualize: "Visualize / Analyze", signals: "Shared signals", openTasks: "Open tasks", completedTasks: "Completed tasks", focusMinutes: "Focus minutes", relationships: "Relationships", personalization: "Personalization", makeItYours: "Make it yours", appName: "App name", language: "Language", english: "English (Canada)", french: "French (Canada)", save: "Save", presentationHint: "Presentation settings are inert and do not change canonical IDs.", capture: "Capture", getItOut: "Get it out of your head", kind: "Kind", note: "Note", task: "Task", observation: "Observation", space: "Space", acquireHeading: "Stage an import", stageImport: "Stage source", acceptStaged: "Accept staged records", acquirePlaceholder: "Paste text, JSON, CSV, or a URL", acquireHint: "Review candidates before canonical records are created; repeated source candidates are skipped.", stagedMessage: (count, warnings) => `Staged ${count} candidate(s)${warnings ? ` with ${warnings} warning(s)` : ""}.`, personal: "Personal", household: "Household", work: "Work", captureContent: "Capture content", capturePlaceholder: "Capture a thought, task, observation, or question.", captureHint: "Stored locally as one canonical record; new captures enter Review.", saveCapture: "Save capture", searchExplore: "Search / Explore", findCaptures: "Find across your captures", searchTerms: "Search terms", searchPlaceholder: "Try a word or phrase", search: "Search", clear: "Clear", triage: "Triage / Clarify", reviewInbox: "Review your inbox", inboxCount: "inbox count", inboxClear: "Your capture inbox is clear.", markReviewed: "Mark reviewed", relate: "Relate", connectWithoutCopying: "Connect without copying", sourceRecord: "Source record", targetRecord: "Target record", relationship: "Relationship", createLink: "Create link", relationshipHint: "Links are reference records; the source records stay single-owner.", timeObserve: "Time / Observe", track: "Track / Observe", trackHeading: "Record a measurement", metricName: "Metric name", value: "Value", unit: "Unit", trackPlaceholder: "For example: sleep quality", saveObservation: "Save observation", trackHint: "Numbers remain source observations and can be reused by views without a second owner.", trackSaved: (name) => `Saved ${name} as an observation.`, domains: "Domains", financeHeading: "Capture an expense", merchant: "Merchant", currency: "Currency", saveExpense: "Save expense", financeHint: "Money is stored as exact minor units with an explicit currency.", healthHeading: "Capture a health measurement", subject: "Subject identity", optionalNote: "Note (optional)", saveMeasurement: "Save measurement", healthHint: "Health observations require explicit subject identity and remain user observations.", expenseSaved: "Expense saved.", measurementSaved: "Health measurement saved.", focusHeading: "Record a focus session", focusHint: "The session becomes a canonical observation when you stop it. No background tracking is used.", startFocus: "Start focus", stopFocus: "Stop focus", noActiveSession: "No active session.", canonicalRecords: "Canonical records", recentCaptures: "Recent captures", recordCount: "record count", nothingCaptured: "Nothing captured yet.", noMatching: "No matching captures.", taskDone: "task - done", complete: "Complete", undo: "Undo", archive: "Archive", archivedRecords: "Archived records", showArchived: "Show archived", hideArchived: "Hide archived", noArchived: "No archived records.", restore: "Restore", recovery: "Recovery", keepPortable: "Keep a portable copy", recoveryHint: "Vault export is the first off-origin recovery path. Import validates the format before writing.", exportVault: "Export Vault", exportDiagnostics: "Export diagnostics", repairSearch: "Repair search index", importVault: "Import Vault", attachArtifact: "Attach artifact", footerPhase0: "Phase 0 foundation", footerOptional: "AI and sync are optional and not active.", themeLight: "Light theme", themeDark: "AMOLED dark", capturePicture: "Capture something to start your picture.", atLeastTwo: "Capture at least two records before creating a link.", linkCreated: "Created a reference link without duplicating either record.", showingAll: "Showing all active records.", exportMessage: (count) => `Exported ${count} record(s).`, diagnosticsMessage: "Exported privacy-minimized diagnostics; canonical content and credentials are excluded.", searchRepairMessage: "Rebuilt the derived search index; canonical records were not changed.", savedName: (name) => `Saved ${name}. Canonical identities are unchanged.`, importedMessage: (imported, skipped, conflicts) => `Imported ${imported} record(s); skipped ${skipped}${conflicts ? `, conflicts ${conflicts}` : ""}.`, attachedMessage: (name, size) => `Attached ${name} (${size} bytes).`, startedMessage: (time) => `Started ${time}.`, savedFocusMessage: (minutes) => `Saved a ${minutes}-minute focus observation.`, healthMessage: (active, archived, history, artifacts, index) => `${active} active, ${archived} archived, ${history} revision snapshot(s), ${artifacts} artifact payload(s); search index ${index}.`, resultMessage: (count, index) => `${count} result(s); derived index ${index}.`
};

const french: UiCopy = {
  ...english,
  scopeWithoutCopying: "Associer des dossiers a un espace", assignToSpace: "Dossier", filterSpace: "Voir l'espace", allSpaces: "Tous les espaces", addMembership: "Ajouter a l'espace", membershipCreated: (space) => `Dossier ajoute a ${space}; le dossier canonique n'a pas ete copie.`, historyHeading: "Historique des revisions", historyEntry: (revision, time, changes) => `Revision ${revision} - ${time}; changements: ${changes}`,
...
const timeCopy: Record<PresentationLocale, TimeCopy> = {
  "en-CA": { reminders: "Attention / reminders", noDue: "Nothing is due.", dueOnResume: "Due on resume", deliveryLimited: "Delivery is opportunistic; the reminder remains canonical.", reminderHeading: "Create a reminder", reminderTitle: "Reminder title", reminderDueAt: "Due date and time", saveReminder: "Save reminder", reminderSaved: "Reminder saved.", reminderHint: "The due state remains canonical; exact closed-app delivery is not promised." },
  "fr-CA": { reminders: "Attention / rappels", noDue: "Rien n'est du.", dueOnResume: "Du a la reprise", deliveryLimited: "La livraison est opportuniste; le rappel reste canonique.", reminderHeading: "Creer un rappel", reminderTitle: "Titre du rappel", reminderDueAt: "Date et heure d'echeance", saveReminder: "Enregistrer le rappel", reminderSaved: "Rappel enregistre.", reminderHint: "L'etat d'echeance reste canonique; aucune livraison exacte hors application n'est promise." }
};

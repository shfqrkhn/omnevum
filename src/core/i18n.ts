...
export interface UiCopy {
  ...
  importPreviewMessage: (records: number, history: number, artifacts: number, imported: number, skipped: number, conflicts: number, hasPresentation: boolean, packageStates: number) => string;
...
const english: UiCopy = {
  ...
  importPreviewMessage: (records, history, artifacts, imported, skipped, conflicts, hasPresentation, packageStates) => `Vault preview: ${records} record(s), ${history} history entr${history === 1 ? "y" : "ies"}, ${artifacts} artifact payload(s); ${imported} will import, ${skipped} will skip${conflicts ? `, ${conflicts} conflict(s)` : ""}${hasPresentation ? "; presentation settings included" : ""}${packageStates ? `; ${packageStates} package state(s) included` : ""}. Proceed?`,
...
const french: UiCopy = {
  ...
  importPreviewMessage: (records, history, artifacts, imported, skipped, conflicts, hasPresentation, packageStates) => `Apercu du Vault: ${records} dossier(s), ${history} entree(s) d'historique, ${artifacts} artefact(s); ${imported} seront importes, ${skipped} ignores${conflicts ? `, ${conflicts} conflit(s)` : ""}${hasPresentation ? "; reglages de presentation inclus" : ""}${packageStates ? `; ${packageStates} etat(s) de paquet inclus` : ""}. Continuer?`,

...
export interface UiCopy {
  ...
  importPreviewMessage: (records: number, history: number, artifacts: number, imported: number, skipped: number, conflicts: number, hasPresentation: boolean) => string;
  importCancelled: string;
  attachedMessage: (name: string, size: number) => string; startedMessage: (time: string) => string;
  savedFocusMessage: (minutes: number) => string; healthMessage: (active: number, archived: number, history: number, artifacts: number, index: string, pressure?: "NORMAL" | "ELEVATED") => string;

...
export interface UiCopy {
  ...
  assistant?: string; assistantHeading?: string; assistantHint?: string; assistantProvider?: string; assistantScope?: string; assistantStandaloneScope?: string; assistantRecordScope?: (recordId: string, relationships: number) => string; assistantDisclosure?: string; assistantDisabled?: string; assistantQuestion?: string; assistantQuestionPlaceholder?: string; assistantSubmit?: string; assistantClearScope?: string; assistantOpen?: string;
  ...
  domains: string; financeHeading: string; merchant: string; currency: string; saveExpense: string; financeHint: string; financePlanHeading: string; financePlanKind: string; financeResource: string; financeGoal: string; financePlanLabel: string; financePlanAmount: string; financeGoalDate: string; financeMonthlySurplus: string; financeHardConstraint: string; financePlanHint: string; saveFinancePlan: string; financePlanSaved: (kind: string) => string; financeGoalStatus: (goals: number, conflicts: number) => string; financeGoalProgress: (label: string, funded: string, target: string, remaining: string, conflict: boolean) => string; financeImportHeading: string; financeFile: string; financeAccount: string; financeOpening: string; financeClosing: string; financeImport: string; financeImportHint: string; financeImportResult: (created: number, existing: number, duplicates: number, conflicts: number, reconciliation: "MATCH" | "MISMATCH" | "INCOMPLETE") => string; financeBatchPreview: (sources: number, rows: number, exceptions: number) => string; financeBatchResult: (sources: number, created: number, existing: number, exceptions: number) => string; financeAnalysisResult: (income: string, spending: string, net: string, pending: string, fees: string) => string; financeCorrectionHeading: string; financeCorrectionRecord: string; financeCorrectionAmount: string; financeCorrectionEssential: string; financeCorrectionUnchanged: string; financeCorrectionYes: string; financeCorrectionNo: string; financeCorrectionHint: string; financeCorrectionSubmit: string; financeCorrectionSaved: (revision: number) => string; financeDashboard: string; financeNoData: string; financeIncome: string; financeSpending: string; financeNet: string; financePending: string; financeQuality: (status: string, limitations: number) => string; financeReviewCases: (count: number) => string; financeGraphStatus: (nodes: number, edges: number, invalidated: number) => string; financeAllocationConflicts: (count: number) => string; financeTransferStatus: (matched: number, unresolved: number) => string; financeCrossDomainStatus: (travel: number, compensation: number, months: number, affected: number, incomplete: number) => string; financeFundingStatus: (shortfall: string, goals: number, alternatives: number, hardConflict: boolean) => string; financeFundingReview: (alternatives: number) => string; financeFundingReviewHint: string; financeFundingAlternative: (label: string, total: string, shortfalls: number) => string; financeFundingContribution: (label: string, amount: string, hard: boolean) => string; financeFundingShortfall: (label: string, amount: string) => string; financeFireStatus: (scenarios: number, limitations: number) => string; healthHeading: string; subject: string; optionalNote: string; saveMeasurement: string; healthHint: string; expenseSaved: string; measurementSaved: string;
  financeImpactStatus: (owners: number, lenses: number, invalidated: number, recomputed: number) => string;
...
const financeEnglish: Pick<UiCopy, "financeImportHeading" | "financeFile" | "financeAccount" | "financeOpening" | "financeClosing" | "financeImport" | "financeImportHint" | "financeImportResult" | "financeBatchPreview" | "financeBatchResult" | "financeAnalysisResult" | "financeCorrectionHeading" | "financeCorrectionRecord" | "financeCorrectionAmount" | "financeCorrectionEssential" | "financeCorrectionUnchanged" | "financeCorrectionYes" | "financeCorrectionNo" | "financeCorrectionHint" | "financeCorrectionSubmit" | "financeCorrectionSaved"> = {
  ...
  financeBatchPreview: (sources, rows, exceptions) => `Finance batch: ${sources} source(s), ${rows} transaction row(s), ${exceptions} exception(s) require review.`,
  financeBatchResult: (sources, created, existing, exceptions) => `Finance batch accepted: ${sources} source(s), ${created} new transaction(s), ${existing} existing transaction(s), ${exceptions} bounded exception(s).`,
  ...
  financeCorrectionHeading: "Correct an imported Finance transaction",
  financeCorrectionRecord: "Imported transaction",
  financeCorrectionAmount: "Correct amount (optional)",
  financeCorrectionEssential: "Essential classification",
  financeCorrectionUnchanged: "Leave unchanged",
  financeCorrectionYes: "Essential",
  financeCorrectionNo: "Not essential",
  financeCorrectionHint: "Corrections create one canonical revision, preserve source lineage, mark the record corrected, and refresh all read-only Finance projections.",
  financeCorrectionSubmit: "Save correction",
  financeCorrectionSaved: (revision) => `Finance correction saved as canonical revision ${revision}; dependent projections were refreshed.`
};

const financeFrench: Pick<UiCopy, "financeImportHeading" | "financeFile" | "financeAccount" | "financeOpening" | "financeClosing" | "financeImport" | "financeImportHint" | "financeImportResult" | "financeBatchPreview" | "financeBatchResult" | "financeAnalysisResult" | "financeCorrectionHeading" | "financeCorrectionRecord" | "financeCorrectionAmount" | "financeCorrectionEssential" | "financeCorrectionUnchanged" | "financeCorrectionYes" | "financeCorrectionNo" | "financeCorrectionHint" | "financeCorrectionSubmit" | "financeCorrectionSaved"> = {
  ...
  financeBatchPreview: (sources, rows, exceptions) => `Lot financier: ${sources} source(s), ${rows} ligne(s) de transaction, ${exceptions} exception(s) a revoir.`,
  financeBatchResult: (sources, created, existing, exceptions) => `Lot financier accepte: ${sources} source(s), ${created} nouvelle(s) transaction(s), ${existing} transaction(s) deja presente(s), ${exceptions} exception(s) delimitee(s).`,
  ...
  financeCorrectionHeading: "Corriger une transaction financiere importee",
  financeCorrectionRecord: "Transaction importee",
  financeCorrectionAmount: "Montant corrige (facultatif)",
  financeCorrectionEssential: "Classification essentielle",
  financeCorrectionUnchanged: "Laisser inchange",
  financeCorrectionYes: "Essentielle",
  financeCorrectionNo: "Non essentielle",
  financeCorrectionHint: "La correction cree une revision canonique, preserve la provenance source, marque le dossier corrige et actualise les projections financieres en lecture seule.",
  financeCorrectionSubmit: "Enregistrer la correction",
  financeCorrectionSaved: (revision) => `Correction financiere enregistree comme revision canonique ${revision}; les projections dependantes ont ete actualisees.`
};

const financePlanEnglish: Pick<UiCopy, "financePlanHeading" | "financePlanKind" | "financeResource" | "financeGoal" | "financePlanLabel" | "financePlanAmount" | "financeGoalDate" | "financeMonthlySurplus" | "financeHardConstraint" | "financePlanHint" | "saveFinancePlan" | "financePlanSaved" | "financeGoalStatus" | "financeGoalProgress"> = {
  ...
  financeHardConstraint: "Protect this goal's required contribution as a hard constraint",
...
const financePlanFrench: Pick<UiCopy, "financePlanHeading" | "financePlanKind" | "financeResource" | "financeGoal" | "financePlanLabel" | "financePlanAmount" | "financeGoalDate" | "financeMonthlySurplus" | "financeHardConstraint" | "financePlanHint" | "saveFinancePlan" | "financePlanSaved" | "financeGoalStatus" | "financeGoalProgress"> = {
  ...
  financeHardConstraint: "Proteger la contribution requise de cet objectif comme contrainte forte",
...
const financeDashboardEnglish: Pick<UiCopy, "financeDashboard" | "financeNoData" | "financeIncome" | "financeSpending" | "financeNet" | "financePending" | "financeQuality" | "financeReviewCases" | "financeGraphStatus" | "financeImpactStatus" | "financeAllocationConflicts" | "financeTransferStatus" | "financeCrossDomainStatus" | "financeFundingStatus" | "financeFundingReview" | "financeFundingReviewHint" | "financeFundingAlternative" | "financeFundingContribution" | "financeFundingShortfall" | "financeFireStatus"> = {
  ...
  financeImpactStatus: (owners, lenses, invalidated, recomputed) => `Change impact: ${owners} owner domain(s) across ${lenses} lens(es); ${invalidated} uncertain projection(s) invalidated and ${recomputed} safe projection(s) recomputed.`,
  ...
  financeCrossDomainStatus: (travel, compensation, months, affected, incomplete) => `Authorized cross-domain projection: ${travel} Travel plan(s), ${compensation} Work compensation change(s), ${months} forecast month(s), ${affected} affected Finance projection(s)${incomplete ? `, ${incomplete} incomplete signal(s)` : ""}.`,
  financeFundingStatus: (shortfall, goals, alternatives, hardConflict) => `Funding conflict: ${shortfall} monthly shortfall across ${goals} goal(s); ${hardConflict ? "hard constraints also conflict" : `${alternatives} modeled alternative(s) preserve hard constraints for explicit review`}.`,
  financeFundingReview: (alternatives) => `Review ${alternatives} modeled funding alternative(s)`,
  financeFundingReviewHint: "Alternatives are read-only models. No canonical goal, priority, allocation, or balance changes are applied.",
  financeFundingAlternative: (label, total, shortfalls) => `${label}: ${total} total monthly contribution; ${shortfalls} goal shortfall(s).`,
  financeFundingContribution: (label, amount, hard) => `${label}: ${amount}${hard ? " (hard constraint)" : ""}`,
  financeFundingShortfall: (label, amount) => `${label}: shortfall ${amount}`,
  financeFireStatus: (scenarios, limitations) => `FIRE/retirement models: ${scenarios} modeled scenario(s); ${limitations} incomplete scenario(s) remain explicit and are not advice.`
};

const financeDashboardFrench: Pick<UiCopy, "financeDashboard" | "financeNoData" | "financeIncome" | "financeSpending" | "financeNet" | "financePending" | "financeQuality" | "financeReviewCases" | "financeGraphStatus" | "financeImpactStatus" | "financeAllocationConflicts" | "financeTransferStatus" | "financeCrossDomainStatus" | "financeFundingStatus" | "financeFundingReview" | "financeFundingReviewHint" | "financeFundingAlternative" | "financeFundingContribution" | "financeFundingShortfall" | "financeFireStatus"> = {
  ...
  financeImpactStatus: (owners, lenses, invalidated, recomputed) => `Impact du changement: ${owners} domaine(s) proprietaire(s) sur ${lenses} lentille(s); ${invalidated} projection(s) incertaine(s) invalidee(s) et ${recomputed} projection(s) sure(s) recalculee(s).`,
  ...
  financeCrossDomainStatus: (travel, compensation, months, affected, incomplete) => `Projection interdomaines autorisee: ${travel} plan(s) de voyage, ${compensation} changement(s) de remuneration, ${months} mois de prevision, ${affected} projection(s) financiere(s) affectee(s)${incomplete ? `, ${incomplete} signal(s) incomplet(s)` : ""}.`,
  financeFundingStatus: (shortfall, goals, alternatives, hardConflict) => `Conflit de financement: deficit mensuel de ${shortfall} pour ${goals} objectif(s); ${hardConflict ? "les contraintes fortes sont aussi en conflit" : `${alternatives} alternative(s) modelee(s) preservent les contraintes fortes pour revue explicite`}.`,
  financeFundingReview: (alternatives) => `Revoir ${alternatives} alternative(s) de financement modelee(s)`,
  financeFundingReviewHint: "Les alternatives sont des modeles en lecture seule. Aucun objectif, priorite, allocation ou solde canonique n'est modifie.",
  financeFundingAlternative: (label, total, shortfalls) => `${label}: ${total} de contribution mensuelle totale; ${shortfalls} deficit(s) d'objectif.`,
  financeFundingContribution: (label, amount, hard) => `${label}: ${amount}${hard ? " (contrainte forte)" : ""}`,
  financeFundingShortfall: (label, amount) => `${label}: deficit ${amount}`,
  financeFireStatus: (scenarios, limitations) => `Modeles FIRE/retraite: ${scenarios} scenario(s) modele(s); ${limitations} scenario(s) incomplet(s) restent explicites et ne constituent pas un conseil.`
};
...
const english: UiCopy = {
  ...
  assistant: "Assistant", assistantHeading: "Ask within a bounded scope", assistantHint: "The Assistant is a shared capability surface, not a data owner. Scope stays explicit and narrowable.", assistantProvider: "Provider / route: none configured", assistantScope: "Scope", assistantStandaloneScope: "Standalone: no canonical records are bound.", assistantRecordScope: (recordId, relationships) => `Record ${recordId} plus ${relationships} authorized relationship(s).`, assistantDisclosure: "Disclosure: local-only; nothing leaves this browser.", assistantDisabled: "AI routes are disabled or unavailable. Core capture, search, review, and recovery remain available.", assistantQuestion: "Question or objective", assistantQuestionPlaceholder: "Ask a question; no request will be sent while AI is disabled.", assistantSubmit: "Send to Assistant", assistantClearScope: "Clear record scope", assistantOpen: "Ask Assistant about this record",
...
const assistantFrench: AssistantCopy = {

...
The current `npm run ci` build at source revision `3a24a21` is bound in `docs/control/release-evidence.json` by artifact digest `4be73f18fd2b04e5c33a4a64c96374f4621fa3e270a0e7148b83920954e554c2`; its stamped service-worker cache is `omnevum-shell-e7f2507d84d604b1`. The daily MVP triage surface now presents localized Review, Defer, Clarify, Keep as reference, and Archive actions. Review/defer/clarify/reference updates carry the record's expected revision through the canonical CommandBus update path; archive uses the canonical archive command, stale reference disposition is cleared by later triage changes, and failures report that no state change was confirmed. `src/core/domain.test.ts` covers preservation of explicit triage states, the reference disposition, and fail-safe defaulting of malformed values to the inbox.

On the same artifact at `http://127.0.0.1:4220/` in the Codex In-app Browser Chromium surface, a fresh capture entered the inbox; the new `Keep as reference` action cleared the inbox, advanced the record to revision 2 while keeping it active, and retained a search-healthy health line. The parent artifact receipt separately observed Defer, Clarify, Review, and Archive; this current-artifact run binds the new reference path to the current digest.

This is bounded source/unit and local Chromium interaction evidence only. Split/link/route breadth, defer scheduling semantics, broader accessibility/target qualification, and complete triage acceptance remain open.

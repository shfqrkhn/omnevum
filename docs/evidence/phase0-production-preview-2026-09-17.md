...
Source revision `3525922` adds an opt-in, repository-owned Vite preview middleware and Windows-safe launcher: `npm run preview:effect-fixture -- 4230`. The middleware is enabled only when `OMNEVUM_EFFECT_FIXTURE=1`, serves `/__omnevum/effect/action` from the same origin as the built preview, and is not included in the production `dist/` artifact. The standalone loopback fixture and the preview middleware share one handler/state owner, so the direct CORS and same-origin receipts cannot drift.

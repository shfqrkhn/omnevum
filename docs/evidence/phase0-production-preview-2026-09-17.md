...
On 2026-09-18, the exact current artifact (`13b1c02148d04502833fd642d9f78edc5f84e0b2`, artifact digest `6a7132b3a05cc1397bf31dc495cf4c03ce8441fbf9bccaae1112a3675032098b`, service-worker cache `omnevum-shell-cf6528a226538fc4`) was served at `http://localhost:4188/?platform-limited=1` in the Codex In-app Browser Chromium surface (`Chrome/153.0.0.0`). Read-only runtime inspection confirmed `display-mode = browser`, `document.title = Omnevum - Life, in context.`, zero canonical records, and an empty app-origin warning/error log. The presentation surface visibly reported `PLATFORM_LIMITED`, explained that browser/OS-controlled name, icon, and splash metadata may be captured at install time, and named the host-supported refresh/re-add/reinstall path while stating that the presentation profile does not change canonical data or technical install identity.
